...
if(isset($_SESSION['username']) && isset($_SESSION['level']))
{
  ...
  $showData = "SELECT id, ma_monhoc, ten_monhoc, so_tin FROM mon_hoc ORDER BY id DESC";
...
//   $maMon = "MH" . time();
  ...
  if(isset($_POST['save']))
  {
    ...
    // validate
    if(empty($maMon))
      $error['maMon'] = 'Vui lòng nhập <b> mã môn học </b>';
    if(empty($tenMon))
      $error['tenMon'] = 'Vui lòng nhập <b> tên môn học </b>';

    if(!$error)
    {
      $showMess = true;
      // them vao db
      $insert = "INSERT INTO mon_hoc (ma_monhoc, ten_monhoc, so_tin, nguoi_tao, ngay_tao) VALUES('$maMon', '$tenMon', '$soTin', '$nguoiTao', '$ngayTao')";
      $result = mysqli_query($conn, $insert);
      echo $insert;
      if($result)
      {
        $success['success'] = 'Tạo môn học thành công.';
        echo '<script>setTimeout("window.location=\'mon_hoc.php?p=group&a=add-group\'",1000);</script>';
      }
    }
 }
     // delete record
  if(isset($_POST['delete']))
  {
    ...
    $id = $_POST['idPosition'];
    $delete = "DELETE FROM mon_hoc WHERE id = $id";
    mysqli_query($conn, $delete);
    $success['success'] = 'Xóa môn học thành công.';
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Tạo môn học sinh viên</li>
      </ol>
    </section>
              ...
              <form action="" method="POST">
                <div class="row">
                  ...
                  <div class="form-group">
                      <label for="exampleInputEmail1">Mã môn học <span style="color: red;">*</span>: </label>
                      <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Nhập mã môn học" name="maMon" value="<?php echo isset($_POST['maMon']) ? $_POST['maMon'] : ''; ?>">
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail1">Tên môn học <span style="color: red;">*</span>: </label>
                      <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Nhập tên môn học" name="tenMon" value="<?php echo isset($_POST['tenMon']) ? $_POST['tenMon'] : ''; ?>">
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail1">Số tín: </label>
                      <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Nhập số tín môn học" name="soTin" value="<?php echo isset($_POST['soTin']) ? $_POST['soTin'] : ''; ?>">
                    ...
                    <?php 
                      if($_SESSION['level'] == 1)
                        echo "<button type='submit' class='btn btn-primary' name='save'><i class='fa fa-plus'></i> Tạo môn học sinh viên</button>";
                    ?>
                  </div>
                  <!-- /.col -->
                </div>
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  ?>
                      <tr>
                        ...
                        <td><?php echo $arrS['ma_monhoc']; ?></td>
                        <td><?php echo $arrS['ten_monhoc']; ?></td>    
                        <td><?php echo $arrS['so_tin']; ?></td>                
                        <td>
                          <?php 
                            if($row_acc['quyen'] == 1)
                            {
